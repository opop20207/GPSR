...
def talk():
    board_list=query_db('select * from board')
    return render_template('/talk/talk.html', board_list=board_list)

@app.route('/talk/<board_name>', methods=['GET', 'POST'])
def board_view(board_name):
    board = query_db('select * from problem where board_name is ?', [board_name])
...
@app.route('/geonguprincesssecretroom/view_board')
def admin_view_board():
    return render_template('/admin/admin_view_board.html',boards=query_db('''
...
@app.route('/geonguprincesssecretroom/addB',methods=['POST'])
def addB():
    error=None
    if request.method == 'POST':
        if not request.form['board_name']:
            error="You have to enter a board name"
        else:
            g.db.execute('insert into board(board_name) values(?)', [request.form['board_name']])
            g.db.commit()
            return redirect(url_for('admin_view_board'))
    return render_template('admin/admin_add_board.html', error=error)

@app.route('/geonguprincesssecretroom/add_problem')
def admin_add_problem():
    return render_template('/admin/admin_add_problem.html', error=None)

@app.route('/geonguprincesssecretroom/add_board')
def admin_add_board():
    return render_template('/admin/admin_add_board.html', error=None)
...
@app.route('/geonguprincesssecretroom/delete_board/<board_num>')
def admin_delete_board(board_num):
